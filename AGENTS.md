...
> **變更記錄 (Changelog)**
> - 2026-07-26: 正式站完成 GOODJOB DESIGN 鹽白 CI 收尾：官方四色、淡方格、左文右橫圖 Hero、相簿預覽與詳情 modal、四類服務頁各 4 個代表案例、完整品牌字標與首頁兩行敘述均已部署；正式資料維持 `articles=64`、`article_images=632`、`accounts=3`。GitHub 專案說明、設計文件與交接狀態同步整理。
...
> - 2026-04-03: 初次建立 AGENTS.md。

## 專案職責

品牌活動、主題場景、展場空間的作品集展示站，搭配輕量 REST API 做內容管理。主站採 GOODJOB DESIGN 鹽白編輯風 CI；分類帽保留主題式深色內容。
...
- **伺服器:** `server.py` — Python HTTP server
...
- **SPA 邏輯:** `assets/site.js` — fetch `/api/articles`、渲染章節式相簿、詳情 modal
- **樣式:** `assets/site.css` — GOODJOB DESIGN CI 與全站響應式樣式
...
只有需要用舊 JSON 備份 seed 本機暫存 DB 時才加 `GOODJOB_ALLOW_JSON_SEED=1`；production 禁止設定。
...
| 方法 | 端點 | 權限 | 說明 |
|------|------|------|------|
| GET | `/api/quotes` | `quotes.manage` | 列出可管理報價單 |
| PUT | `/api/quotes/{id}` | `quotes.manage` | 更新報價單狀態與存取設定 |
| DELETE | `/api/quotes/{id}` | `quotes.manage` | 封存報價單 |
...
**驗證方式：** HTTP Basic Auth，salted SHA-256 hash。Production 帳號由 PostgreSQL `accounts`／`account_permissions` 管理；SQLite 本機模式沿用同一 schema，舊版 `config.json` 只保留單一 admin fallback。

## 權限與帳號模型

歷史備份 `data/accounts.json`（git-ignored）結構如下；正式資料以 PostgreSQL 為準：
```json
{
  "accounts": [{
    "username": "...",
...
**6 種 permission：** `articles.read` / `articles.write` / `articles.delete` / `uploads.write` / `accounts.manage` / `quotes.manage`
...
**舊備份 `data/articles.json`** 結構：`{ "articles": [ ... ] }`。正式資料請以 PostgreSQL schema 為準。
...
前端仍需瀏覽器視覺檢查；部署後驗證公開頁、`/controlcenter/`、service 與 PostgreSQL 三項筆數。
...
**Q: 如何新增作品？**
A: `/controlcenter/` CMS 後台上傳。圖片自動轉 WebP、上 R2，作品與圖片 URL 寫入 PostgreSQL。
...
| 檔案 | 行數 | 用途 |
|------|------|------|
| `server.py` | — | HTTP 伺服器 + REST API + PostgreSQL + R2 upload + accounts + quotes + SSR |
| `assets/site.js` | — | SPA 前端邏輯 |
